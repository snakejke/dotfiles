...
updateicon() { \
    if [ "$1" = "" ]; then
        notify-send -u low "🛑 Recording stopped" "File saved to ~/"
    else
...
videoselected() {
    # Получаем координаты области для записи
    slop -f "%x %y %w %h" > /tmp/slop
    read -r X Y W H < /tmp/slop
    rm /tmp/slop
    
    # Запускаем overlay_frame с полученными координатами
    overlay_frame "$X" "$Y" "$W" "$H" &
    echo $! > /tmp/overlaypid

    # Запускаем запись области
    ffmpeg \
    -f x11grab \
    -framerate 30 \
    -video_size "${W}x${H}" \
